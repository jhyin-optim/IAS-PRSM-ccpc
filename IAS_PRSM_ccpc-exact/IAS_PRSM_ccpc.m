...
for k = 1:paras.MAX_ITER
    ...
    rho_xold_h   = rho*baru + paras.barA'*(lambda - beta* diff_xy) ;%==============
    ...
    soft_y = beta/db*barAx + delta/db*barv - 1/db*lambda_mid;  %===========================  
    y      = wthresh(soft_y,'s', soft_para);
    
    %soft_y = barAx - lambda_mid/beta;  %===========================  
    %y      = wthresh(soft_y,'s', soft_para);
