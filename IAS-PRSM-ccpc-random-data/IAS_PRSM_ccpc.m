...
for k = 1:paras.MAX_ITER
    ...
    rho_uold_h   = rho*baru + paras.barA'*(lambda - beta* diff_xy) ;
    ...
    for t = 1:M_k  
       ...
       %% ------------ update x-subproblem -------------
        temp_gs = gamma_t*temp_sigma*x_breve + rho_uold_h - d_t; 
    ...
    barAx_old = barAx ;
    barAx = paras.barA*x ;
    lambda_mid = lambda - const_sb0*(barAx - y);
    
    %% y-update 
    barv = (1 - theta_y)*y + theta_y*v_ag;
    soft_y = beta/gb*barAx + gamma/gb*barv - lambda_mid/gb;  %===========================  
    ...
    %% black_rho-update  %          
            %===============================      
    %black_rho = black_rho_ag - black_dk;
