function [y1,dn1] = CG(step_par1,step_par2,rho,x,y,x_old,lambda_mid,baru,barv,u_ag,v_ag,vn1,method,paras)
% B = 2*lambda_1*A'tA + (beta + eta1)*I
...
A = paras.barA;
AtA = A'*A;
r0 = 2*lambda_1*AtA*y + ( beta + delta)*y - beta*x - delta*barv + lambda_mid;
p0 = - r0;
r0tr0 = r0'*r0;
y1 = y;
...
switch method
    ...
    btkkc = beta2*(theta_y*delta*normxbv_old^2 + kappa*delta*normxbv^2 + constant_c2*delta*normybyy^2);
    bcn = beta3*constant_c0*normMw^2;
    bbb = btnn +  bcn + btkkc; 
    
    while asn > bbb  
            Ap0 = A*p0;
            Cp0 = 2*lambda_1*A'*Ap0 + ( beta + delta)*p0;
            p0Cp0 = p0'*Cp0;
            alphak = r0tr0/p0Cp0;
            y1 = y1 + alphak*p0;
            r0 = r0 + alphak*Cp0;
